...
void InitGPIO(bool setOutput, bool flagOnChangeState, bool setFallingEdge, bool setEnableInterrupt,uint8_t dioNumber)
{
    ...
    setEnableInterrupt?(irq_setmask(irq_getmask()|DIO_INT[dioNumber]),DIOs[dioNumber].enable_write(1)):DIOs[dioNumber].enable_write(0);
}   
...
{  
    ...
    if((pinNumber==RADIO_NSS)){
        ...
        switch(value)
        {
            case 0 : startSPI();    break;
            case 1 : stopSPI();     break;
        }
    }
}
