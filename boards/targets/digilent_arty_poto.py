...
from litex_boards.platforms import digilent_arty_poto
...
class BaseSoC(SoCCore):
    def __init__(self, variant="a7-35", toolchain="vivado", sys_clk_freq=int(100e6),
                 with_lora=True,with_ethernet=False, with_etherbone=False, eth_ip="192.168.1.50",
                 eth_dynamic_ip=False, with_led_chaser=True, with_jtagbone=True,
                 with_spi_flash=False, with_pmod_gpio=False, **kwargs):
        platform = digilent_arty_poto.Platform(variant=variant, toolchain=toolchain)
        ...
        if with_lora:
            ...
            #solution1
            #self.add_gpio(name="dio",pads=platform.request("dio"),with_irq=True)
            ...
            self.add_gpio(name="rst",pads=platform.request("rst"),with_irq=True)
        ...
        # GPIOs ------------------------------------------------------------------------------------
        if with_pmod_gpio:
            platform.add_extension(digilent_arty_poto.raw_pmod_io("pmoda"))
...
def main():
    ...
    if args.sdcard_adapter == "numato":
        soc.platform.add_extension(digilent_arty_poto._numato_sdcard_pmod_io)
    else:
        soc.platform.add_extension(digilent_arty_poto._sdcard_pmod_io)
